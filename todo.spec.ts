...
async function resetServer(seeds: { title: string; done: boolean }[] = []) {
...
test.describe('Todo App', () => {
  ...
  test.describe('initial state', () => {
    ...
    test('add button is disabled and error hidden on load', async ({ page }) => {
  ...
  test.describe('input validation', () => {
    ...
    test('empty input keeps add button disabled and error hidden', async ({ page }) => {
      // Input starts empty — button stays disabled, no error yet
      await expect(page.locator(sel.addBtn)).toBeDisabled();
      await expect(page.locator(sel.errorMsg)).toBeHidden();
    });

    test('whitespace-only input shows error and keeps button disabled', async ({ page }) => {
    ...
    test('form submit with invalid value calls onInvalid (error shown, no item added)', async ({ page }) => {
      // Programmatically submit without filling in anything
      await page.locator('#mainForm').evaluate((f: HTMLFormElement) => f.requestSubmit());
  ...
  test.describe('adding a task', () => {
    ...
    test('add button is disabled again after successful submit', async ({ page }) => {
      await page.fill(sel.input, 'Buy milk');
      await page.locator(sel.addBtn).click();
    ...
    test('new item has a "Done" button (not struck-through)', async ({ page }) => {
  ...
  test.describe('marking an item done', () => {
    ...
    test('clicking Done strikes through the title and removes Done button', async ({ page }) => {
      await nthDoneBtn(page, 0).click();

      // Wait for re-render from server
      const firstSpan = listItems(page).first().locator('span');
      await expect(firstSpan).toHaveClass(/line-through/);
      await expect(nthDoneBtn(page, 0)).toHaveCount(0);
    });

    test('marking one item done does not affect the other', async ({ page }) => {
      await nthDoneBtn(page, 0).click();

      // Second item should still be undone
      const secondSpan = listItems(page).nth(1).locator('span');
      await expect(secondSpan).not.toHaveClass(/line-through/);
  ...
  test.describe('checkbox selection and delete button reactivity', () => {
    ...
    test('selecting one item enables delete button with label "Delete (1)"', async ({ page }) => {
    ...
    test('deselecting all items disables button and resets label to "Delete"', async ({ page }) => {
      await nthCheckbox(page, 0).check();
      await nthCheckbox(page, 1).check();
      await expect(page.locator(sel.deleteBtn)).toHaveText('Delete (2)');
  ...
  test.describe('bulk delete', () => {
    ...
    test('deleting a single selected item removes it from the list', async ({ page }) => {
      await nthCheckbox(page, 0).check();
    ...
    test('bulk deleting two items leaves only the unselected one', async ({ page }) => {
      await nthCheckbox(page, 0).check();
      await nthCheckbox(page, 2).check();
      await page.locator(sel.deleteBtn).click();
    ...
    test('selection clears and delete button resets after delete', async ({ page }) => {
      await nthCheckbox(page, 0).check();
      await nthCheckbox(page, 1).check();
      await page.locator(sel.deleteBtn).click();
  ...
  test.describe('state persistence', () => {
    test('items seeded on the server appear after a fresh page load', async ({ page }) => {
      await resetServer([
      ...
      // Cherry is done — struck through, no Done button
      const cherrySpan = listItems(page).first().locator('span');
      await expect(cherrySpan).toHaveClass(/line-through/);
      await expect(nthDoneBtn(page, 0)).toHaveCount(0);
